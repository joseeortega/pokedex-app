...
export class MainPokemonListService implements OnDestroy {
  ...
  getInitialPokemons(): void {
    ...
    this.pokedexDataService.getPokemons(this.pagination.offset, this.pagination.limit).pipe(
      takeUntil(this.destroy$)
    ).subscribe((response: any) => {
      this.pokemons.next(response.results.map(pokemon => {
        return {
          id: null,
          name: pokemon.name,
  ...
  getMorePokemons(): void {
    ...
    if (!this.listCompleted) {
      ...
      this.pokedexDataService.getPokemons(this.pagination.offset, this.pagination.limit, headers)
        .pipe(
          takeUntil(this.destroy$)
        ).subscribe((response: any) => {
          const responsePokemons: Pokemon[] = response.results.map(pokemon => {
            return {
              id: null,
              name: pokemon.name,
              imgUrl: PokemonImageResolver.getUrlImageByName(pokemon.name)
